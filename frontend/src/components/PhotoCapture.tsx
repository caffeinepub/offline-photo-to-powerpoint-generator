...
import { Camera, Upload, Plus, X, SwitchCamera } from 'lucide-react';
...
export default function PhotoCapture({ onPhotoAdd }: PhotoCaptureProps) {
  ...
  const [activeTab, setActiveTab] = useState<'camera' | 'upload' | 'bulk'>('camera');
    ...
    Array.from(files).forEach((file) => {
      ...
      reader.onload = (e) => {
        ...
        if (newBulkUploads.length === validFiles) {
          ...
          toast.success(`${validFiles} photo${validFiles > 1 ? 's' : ''} added for bulk upload`);
        }
      };
      reader.readAsDataURL(file);
    });
  ...
  const handleBulkLocationChange = (index: number, value: string) => {
    setBulkUploads((prev) =>
      prev.map((item, i) => (i === index ? { ...item, locationName: value } : item))
    );
  };
  ...
  const handleAddAllBulkPhotos = () => {
    const itemsWithoutLocation = bulkUploads.filter((item) => !item.locationName.trim());
    
    if (itemsWithoutLocation.length > 0) {
      toast.error(`Please enter location names for all ${itemsWithoutLocation.length} photo${itemsWithoutLocation.length > 1 ? 's' : ''}`);
      return;
    }
    ...
    bulkUploads.forEach((item) => {
      onPhotoAdd(item.previewUrl, item.locationName.trim());
    });

    toast.success(`${bulkUploads.length} photo${bulkUploads.length > 1 ? 's' : ''} added successfully`);
  ...
  return (
    <Card className="border-2">
      <CardContent className="pt-6">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'camera' | 'upload' | 'bulk')}>
                  ...
                  <div className="flex gap-2">
                    {!isActive ? (
                      ...
                      <>
                        ...
                        <Button variant="outline" onClick={stopCamera} disabled={cameraLoading}>
                          Stop
                        </Button>
                      </>
                    )}
          ...
          <TabsContent value="upload" className="space-y-4">
            <div className="space-y-4">
              ...
              {previewUrl ? (
                <>
                  ...
                  <Button variant="outline" onClick={handleRemoveUpload} className="w-full">
                    <X className="w-4 h-4 mr-2" />
                    Remove Photo
                  </Button>
                </>
            ...
            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                <Label htmlFor="location-name-upload" className="text-base font-semibold">
                  Location Name
                </Label>
                <Input
          ...
          <TabsContent value="bulk" className="space-y-4">
            <div className="space-y-4">
              ...
              >
                <Upload className="w-12 h-12 text-muted-foreground mb-3" />
                <p className="text-sm font-medium mb-1">Click to select multiple photos</p>
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-sm font-medium">
                      {bulkUploads.length} photo{bulkUploads.length > 1 ? 's' : ''} selected
                    </p>
                  </div>
                    ...
                    <div key={index} className="flex gap-3 p-3 border rounded-lg bg-muted/30">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => handleBulkLocationChange(index, e.target.value)}
                          className="h-9"
                        />
                      </div>
                      <Button
              ...
              {bulkUploads.length > 0 && (
                ...
                >
                  <Plus className="w-5 h-5 mr-2" />
                  Add All {bulkUploads.length} Photo{bulkUploads.length > 1 ? 's' : ''}
                </Button>
              )}
            </div>
          </TabsContent>
