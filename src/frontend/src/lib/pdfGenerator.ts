...
// Helper function to render slide to canvas (async logic extracted to avoid noAsyncPromiseExecutor)
async function renderSlideToCanvasAsync(
  ...
  const availableHeight =
    height - padding * 2 - gap * (rows - 1) - (textHeight + textGap) * rows;
    ...
    await new Promise<void>((resolveImg) => {
      ...
      img.onload = () => {
        ...
        if (metrics.width > maxWidth) {
          while (
            ctx.measureText(`${locationText}...`).width > maxWidth &&
